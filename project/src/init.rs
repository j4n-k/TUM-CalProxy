...
impl AppInit {
    pub fn init() -> Self {
        ...
        let port = env::var("ACTIX_PORT").ok()
            .and_then(|p| p.parse().ok())
            .unwrap_or(8080);

        let listen_ip = env::var("ACTIX_LISTEN").ok()
    ...
    pub fn create_app(&self) -> App<impl ServiceFactory<
        ServiceRequest,
        Config = (),
    ...
    QueryConfig::default()
        .error_handler(|err, _| {
            ...
            if e.starts_with("missing field `") {
                let missing_parameter = e.trim_start_matches("missing field `")
                    .trim_end_matches("`");
